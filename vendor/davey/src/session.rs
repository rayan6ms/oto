...
pub struct DaveSession {
    ...
    privacy_code: String,
    encryptor: Encryptor,
    pending_encryptor_ratchet: Option<HashRatchet>,
...
impl DaveSession {
    ...
    pub fn new(
        ...
        Ok(DaveSession {
            ...
            pending_encryptor_ratchet: None,
    ...
    pub fn reinit(
        ...
        self.pending_encryptor_ratchet = None;
    ...
    fn update_ratchets(&mut self) -> Result<(), UpdateRatchetsError> {
        ...
        // Stage the sender ratchet. DAVE applies receive-side ratchets while preparing
        // an epoch, but the send-side ratchet must not change until Execute Transition.
    ...
    /// Atomically activates the sender ratchet prepared by the latest successful
    /// Commit or Welcome. Call this only for the matching DAVE Execute Transition.
    pub fn execute_transition(&mut self) -> Result<(), UpdateRatchetsError> {
        let Some(ratchet) = self.pending_encryptor_ratchet.take() else {
...
#[cfg(test)]
mod oto_conformance_tests {
    use super::*;
    use crate::OPUS_SILENCE_PACKET;

    const OLD_SECRET: [u8; AES_GCM_128_KEY_BYTES] = [0x11; AES_GCM_128_KEY_BYTES];
    const NEW_SECRET: [u8; AES_GCM_128_KEY_BYTES] = [0x22; AES_GCM_128_KEY_BYTES];

    fn session() -> DaveSession {
        DaveSession::new(NonZeroU16::new(1).unwrap(), 7, 9, None).unwrap()
    }

    fn decrypt(secret: &[u8], frame: &[u8]) -> Result<Vec<u8>, DecryptorDecryptError> {
        let mut decryptor = Decryptor::new();
        decryptor.transition_to_key_ratchet(HashRatchet::new(secret.to_vec()));
        let mut plaintext = vec![0; frame.len()];
        let written = decryptor.decrypt(MediaType::AUDIO, frame, &mut plaintext)?;
        plaintext.truncate(written);
        Ok(plaintext)
    }

    #[test]
    fn exact_outbound_opus_silence_is_encrypted_and_round_trips() {
        let mut session = session();
        session.pending_encryptor_ratchet = Some(HashRatchet::new(OLD_SECRET.to_vec()));
        assert!(!session.is_ready());
        assert!(matches!(
            session.encrypt_opus(&OPUS_SILENCE_PACKET),
            Err(EncryptError::NotReady)
        ));

        session.execute_transition().unwrap();
        let encrypted = session
            .encrypt_opus(&OPUS_SILENCE_PACKET)
            .unwrap()
            .into_owned();
        assert_ne!(encrypted, OPUS_SILENCE_PACKET);
        assert!(encrypted.len() > OPUS_SILENCE_PACKET.len());
    ...
    #[test]
    fn staged_sender_ratchet_changes_only_at_execute() {
        let mut session = session();
        session.pending_encryptor_ratchet = Some(HashRatchet::new(OLD_SECRET.to_vec()));
        session.execute_transition().unwrap();

        session.pending_encryptor_ratchet = Some(HashRatchet::new(NEW_SECRET.to_vec()));
        let before = session.encrypt_opus(b"before").unwrap().into_owned();
        assert_eq!(decrypt(&OLD_SECRET, &before).unwrap(), b"before");
        assert!(decrypt(&NEW_SECRET, &before).is_err());

        session.execute_transition().unwrap();
        let after = session.encrypt_opus(b"after").unwrap().into_owned();
