    ...
    async fn send_payload(&mut self, len: usize, silence: bool) -> Result<SendOutcome, Error> {
        ...
        for attempt in 0..3 {
            ...
            let result = if injected {
                Ok(Err(std::io::Error::from(std::io::ErrorKind::Interrupted)))
            } else {
                timeout(FRAME_PERIOD, transport.socket.send(&self.packet)).await
            };
            ...
            tokio::select! {
                ...
                _ = self.cancel.changed() => return Err(gateway_stopped()),
                _ = self.connection_shutdown.changed() => return Err(gateway_stopped()),
            }
            // Tokio UDP send is cancellation safe: a cancelled Pending send
            // has emitted nothing, and an error also reports no sent datagram.
            // Retry the already encrypted packet, without encrypting again or
            // advancing RTP media time for an unsent frame. Each encryption
            // still consumes a unique nonce; no plaintext is encrypted twice
            // under the same nonce and no new packet is generated by a retry.
        }
        Err(audio_error(
            ErrorKind::SendIo,
            Operation::StartAudio,
            RetryDisposition::Fatal,
            "UDP send failed after bounded recovery",
        ))
    }
