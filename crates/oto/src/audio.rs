...
mod tests {
    ...
    #[ignore = "release-only complete paced DAVE path benchmark"]
    async fn complete_paced_dave_path_benchmark() {
        let warmup = Duration::from_millis(
            std::env::var("OTO_DAVE_PATH_WARMUP_MS")
                .ok()
                .and_then(|value| value.parse().ok())
                .unwrap_or(1_000),
        );
        let measurement = Duration::from_millis(
            std::env::var("OTO_DAVE_PATH_MEASURE_MS")
                .ok()
                .and_then(|value| value.parse().ok())
                .unwrap_or(3_000),
        );
        ...
        let result = serde_json::json!({
            "schemaVersion": 1,
            "benchmarkId": "oto-complete-paced-dave",
            "profile": "release",
        ...
        println!("OTO_COMPLETE_DAVE_PATH_BENCHMARK={result}");
        assert!(frames > 0);
