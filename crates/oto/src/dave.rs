...
mod tests {
    ...
    #[tokio::test]
    #[ignore = "release-only DAVE adapter benchmark"]
    async fn dave_adapter_benchmark() {
        let sessions = benchmark_env("OTO_DAVE_SESSIONS", 12).max(1);
        let encryptions = benchmark_env("OTO_DAVE_ENCRYPTIONS", 10_000).max(1);
        ...
        println!("OTO_DAVE_ADAPTER_BENCHMARK={result}");
    }

    #[test]
    #[ignore = "release-only simulated 48-hour DAVE/RTP/transport-crypto frame-count soak"]
    fn simulated_dave_transport_crypto_soak() {
        let simulated_hours = benchmark_env("OTO_SIMULATED_SOAK_HOURS", 48);
        ...
        let result = json!({
            "schemaVersion": 1,
            "benchmarkId": "oto-simulated-dave-transport-crypto-soak",
            "profile": "release",
        ...
        println!("OTO_SIMULATED_SOAK={result}");
    }
}
