...
mod tests {
    ...
    async fn active_playback_survives_resumable_websocket_loss_without_catch_up() {
        ...
        println!("OTO_LOCAL_RESUME_MILLIS={resume_millis}");
    ...
    #[ignore = "release-only complete non-DAVE path benchmark"]
    async fn complete_non_dave_path_benchmark() {
        let sender_count = benchmark_env("OTO_BENCH_SENDERS", 250);
        let warmup = Duration::from_millis(benchmark_env("OTO_BENCH_WARMUP_MS", 1_000) as u64);
        let measurement =
            Duration::from_millis(benchmark_env("OTO_BENCH_MEASURE_MS", 3_000) as u64);
        let churn_cycles = benchmark_env("OTO_BENCH_CHURN", 32);
        ...
        let result = serde_json::json!({
            "schemaVersion": 1,
            "benchmarkId": "oto-complete-non-dave",
            "profile": "release",
        ...
        println!("OTO_COMPLETE_PATH_BENCHMARK={result}");
    ...
    async fn simulated_24_hour_connected_lifecycle_soak() {
        ...
        oto.config
            ...
            .store(true, std::sync::atomic::Ordering::Release);
        let connection = oto
            .connect(voice_info(&gateway, "lifecycle-soak", "test-token"))
            .await
            .expect("transport reaches DAVE setup");
        gateway
            .try_dave_prepare_transition(1, 0)
